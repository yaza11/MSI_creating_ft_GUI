...
def parse_da_export(line: str, str_x=None, str_y=None):
    ...
    spectrum = Spectrum(mz_val, intensity,
                        mz_precision=MZ_PRECISION, metadata=spot_name)
...
class Spectrum:
    ...
    def set_peaks(self, mz_values, intensity_values):
        ...
        mz_values = np.asarray(
            np.round(mz_values, self._mz_precision), dtype=float)
    ...
    def between(self, start, end, inplace=False):
        ...
        mz_values = spec.mz_values[np.where(
            (spec.mz_values >= start) & (spec.mz_values <= end))]
    ...
    def show(self):
        ...
        min_mz = max(0, math.floor(
            self.mz_values[0] / interval - 1) * interval)
        max_mz = max(0, math.floor(
            self.mz_values[-1] / interval + 1) * interval)
...
def combine_spectrum(spot: list, spectrum: Spectrum, primer_df: pd.DataFrame, normalization='None'):
    ...
    if normalization == 'None':
        spectrum_df = pd.DataFrame(
            spectrum.intensity_values, index=spectrum.mz_values)
    elif normalization == 'median':
        spectrum_df = pd.DataFrame(
            spectrum.n_intensity_values, index=spectrum.mz_values)
    else:
...
def create_bin(spot, spectrum: Spectrum, ref_peaks, tol=10):
    ...
    for mz_val in spectrum.mz_values:
        ...
        if abs(err) <= tol * 1e-6:
            ...
            new_peaks[new_mz] = max(
                spectrum.intensity_at(mz_val), new_peaks[new_mz])
...
def create_feature_table(spectrum_dict: dict, ref_peaks, tol=10, normalization='None') -> \
    ...
    bin_spectrum_dict = {}
    for spectrum_key, spectrum_val in spectrum_dict.items():
        bin_spectrum_dict[spectrum_key] = create_bin(
            spectrum_key, spectrum_val, ref_peaks, tol)

    bin_spectrum_dict = {key: value[1]
                         for key, value in bin_spectrum_dict.items()}

    print("Combining the binned spectrum...")

    primer_df = pd.DataFrame(
        np.zeros(ref_peaks.shape), index=list(ref_peaks))

    primer_df = primer_df.replace(0, np.nan)

    combined_spectrum_dict = {}
    for key, val in bin_spectrum_dict.items():
        combined_spectrum_dict[key] = combine_spectrum(key, val, primer_df,
                                                       normalization=normalization)

    err_dict = {key: value[2]
                for key, value in combined_spectrum_dict.items()}
...
def find_peaks_in_bin(peak_th=None, ref_peaks=None, c_dist=None, peak_picking_method=None):
    ...
    if isinstance(peak_th, float):
        peaks = find_peaks(
            y_val, peak_picking_method=peak_picking_method, peak_th=peak_th)
        ref_peaks[peak_th].extend([round(x_val[i], 4) for i in peaks])

    elif isinstance(peak_th, list):
        for p_th in peak_th:
            peaks = find_peaks(
                y_val, peak_picking_method=peak_picking_method, peak_th=p_th)
...
def get_ref_peaks(spectrum_dict: dict or list, peak_picking_method='prominence', peak_th=0.1):
    ...
    if isinstance(spectrum_dict, dict):
        ...
        for spectrum_dict_item in spectrum_dict:
            mzs_all.extend(
                [spec.mz_values for spec in spectrum_dict_item.values()])
    ...
    mz_bin = range(int(round(np.min(mzs_all), 0)),
                   int(round(np.max(mzs_all), 0)))

    print(
        f'Detecting reference peaks with peak prominence greater than {peak_th}')
...
def search_peak_th(raw_data: dict, peak_th_candidates,
    ...
    for value in ref.values():
        feature_table_sub = feature_table[list(value) + ['x', 'y']].copy()
        err_table_sub = err_table[list(value) + ['x', 'y']].copy()
        ...
        spar.append((feature_table_sub.drop(
            columns=['x', 'y']).to_numpy() == 0).mean())
...
def peak_alignment_evaluation(spectrum_dict: dict, feature_table: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(spectrum_dict) != len(feature_table):
        raise ValueError(
            "This feature table is not derived from the current raw data!")
